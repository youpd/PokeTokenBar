...
counts. The compact Codex row uses `API $… (subscription)` (`API $… (구독)` in Korean) because it is
not an actual Codex subscription charge or invoice. Tool-call fees, regional uplifts, long-context
multipliers, discounts, and pricing changes may make a real API bill differ from this estimate.
